...
where:
    ...
    -d  distro name (debian, ubuntu, mint, kali)
    -r  release name (buster, focal, 20.3, rolling)
    -p  packages
...
if [ "$distro" != "debian" ] && [ "$distro" != "ubuntu" ] && [ "$distro" != "mint" ] && [ "$distro" != "kali" ]; then
    echo "Error: only Debian, Ubuntu, Mint and Kali are supported!";
    exit 1;
else
